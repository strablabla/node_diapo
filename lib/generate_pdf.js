...
function generate_pdf_from_thumbnails(socket, numdiap) {
    ...
    try {
        ...
        // Use deck title for filename if available, otherwise use slides.pdf with timestamp
        const title = deckTitle.getDeckTitle()
        var filename
        if (title) {
            const sanitized = deckTitle.sanitizeTitleForFilename(title)
