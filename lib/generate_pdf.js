...
function generate_pdf_from_thumbnails(socket, numdiap) {
    ...
    try {
        ...
        // Use PDF title for filename if available, otherwise use slides.pdf with timestamp
        const title = pdfTitle.getPdfTitle()
        var filename
        if (title) {
            const sanitized = pdfTitle.sanitizeTitleForFilename(title)
            filename = sanitized + '.pdf'
        } else {
            const timestamp = Date.now()
            filename = 'slides_' + timestamp + '.pdf'
        }
