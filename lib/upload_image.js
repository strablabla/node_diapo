...
function handleFaviconUpload(file, res, reloadConfig) {
    ...
    fs.copyFile(sourcePath, destPath, function(err) {
        ...
        // Update config.json with favicon name
...
function handleBackgroundUpload(file, res, reloadConfig) {
    ...
    fs.copyFile(sourcePath, destPath, function(err) {
        ...
        // Update config_desk.yaml with background name
        var configPath = 'views/config_desk.yaml'
        fs.readFile(configPath, 'utf8', function(err, data) {
            if (err) {
                console.error('Error reading config_desk.yaml:', err)
                return res.status(500).json({ error: 'Failed to read config' })
            }
            ...
            fs.writeFile(configPath, yaml.dump(config_desk), function(err) {
                if (err) {
                    console.error('Error writing config_desk.yaml:', err)
                    return res.status(500).json({ error: 'Failed to update config' })
                }
