...
function setupWebSocket(io, server, dependencies) {
    ...
    var pdfTitle = require('./pdf_title');
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('return', function(data) {       // change html with textarea
            // Handle both old format (string) and new format (object with text and diapo_index)
            var new_text, slide_index;
            if (typeof data === 'string') {
                // Legacy format: just text, use server's state.diapo_index
                new_text = data;
                slide_index = state.diapo_index;
                console.log('⚠️  Legacy save format - using server diapo_index:', slide_index);
            } else {
                // New format: object with text and diapo_index from client
                new_text = data.text;
            ...
            // Update PDF title if we're saving the first slide
            if (slide_index === 0) {
                var title = pdfTitle.extractPdfTitle(new_text);
                if (title) {
            ...
            thumbnails.updateSingleThumbnail(slide_index, port, state.numdiap).catch(err => {
