...
async function create_thumbnails(numdiap, port){
    ...
    for (var i = 0; i < numdiap; i++) {
        await createSingleThumbnail(browser, i, port, numdiap);
    }
...
async function createSingleThumbnail(browser, index_diap, port, numdiap) {
    ...
    try {
        ...
        await page.evaluate((slideIndex, totalSlides) => {
            ...
            // Add or update #num with slide number
            let numElement = document.getElementById('num');
            if (!numElement) {
                numElement = document.createElement('p');
                numElement.id = 'num';
                document.getElementById('content').appendChild(numElement);
            }
            numElement.textContent = slideIndex + '/' + (totalSlides - 1);
            numElement.style.position = 'absolute';
            numElement.style.bottom = '20px';
            numElement.style.right = '30px';
            numElement.style.fontSize = '20px';
            numElement.style.color = '#333';
        }, index_diap, numdiap);
