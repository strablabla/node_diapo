...
async function create_thumbnails(numdiap, port){
    ...
    for (var i = 0; i < numdiap; i++) {
        await createSingleThumbnail(browser, i, port);
    }
...
async function createSingleThumbnail(browser, index_diap, port) {
...
async function updateSingleThumbnail(index_diap, port) {
    /*
    ...
    */
    ...
    await createSingleThumbnail(browser, index_diap, port);
    await browser.close();
